...
const searchPinnedEnd = content.indexOf('</form>\n            </div>\n          </div>');
if (searchPinnedEnd !== -1) {
    const insertPoint1 = searchPinnedEnd + '</form>\n            </div>\n          </div>'.length;
    ...
    content = content.slice(0, insertPoint1) + pinnedFilters + content.slice(insertPoint1);
}

// Insert Standard Filters
const heroHighlightsStart = content.indexOf('<div\n            className="mx-auto mt-6 w-full max-w-5xl rounded-2xl bg-white px-4 py-3');
if (heroHighlightsStart !== -1) {
...
`;
    content = content.slice(0, heroHighlightsStart) + standardFilters + content.slice(heroHighlightsStart);
}
