...
const FALLBACK_BANNER =
  "https://images.unsplash.com/photo-1493558103817-58b2924bce98auto=format&fit=crop&w=1600&q=70";
...
const BlogDetailsPage = () => {
  ...
  const shareLinks = useMemo(() => {
    ...
    return {
      twitter: `https://twitter.com/intent/tweeturl=${url}&text=${text}`,
      facebook: `https://www.facebook.com/sharer/sharer.phpu=${url}`,
          ...
          <nav className="flex items-center gap-1.5 text-xs sm:text-sm text-white/75 mb-5">
            ...
            {post.category.name  (
              <>
                <ChevronRight size={14} className="text-white/50" />
                <Link
                  to={`/blogcategory=${post.category.slug || post.category._id}`}
              ...
              {Array.isArray(post.tags) && post.tags.length  (
                <div className="mt-10 pt-6 border-t border-border-light flex flex-wrap items-center gap-2">
                  ...
                  {post.tags.map((tag) => (
                    <Link
                      key={tag}
                      to={`/blogq=${encodeURIComponent(tag)}`}
